/*
  ...
  Get data from sensors, print on monitor
  and record to SD

  based on
  SD300toMonitorAndSD

  Still incomplete. Need to add acquisition routines
...
void loop() {
  ...
  if (InAcq == true) {
    if (currentMillis - previousMillis >= interval) {
      // save the last acquisition time
      previousMillis = currentMillis;

      //getdata
      // record data showNewData();
    }
