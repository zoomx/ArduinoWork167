...
void decodePluviometer()
{
  ...
  if (encodedBits[9] && encodedBits[10] && !encodedBits[11]
    ...
    */
    //Serial.println("Decode pluviometer");
    unsigned int rain = 0;
    int i;
    for (i = 16; i < 32; i++) {
...
void decodeWindData()
{
  ...
  if (encodedBits[9] && encodedBits[10] && encodedBits[12]
    ...
    windAverageSpeedRaw = windAverageSpeed;
    Serial.print("; ");
    ...
    if (encodedBits[8]) {
      //Serial.print(LANG_BATTERY_REPLACE);
    } else {
      //Serial.print(LANG_BATTERY_OK);
